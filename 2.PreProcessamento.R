...
# Imputacao, encoding, transformacao log das variaveis assimetricas,
# divisao treino/teste ESTRATIFICADA e normalizacao ajustada apenas no treino
# (evita vazamento de informacao do teste para o treino).
# =============================================================================
...
# --- Imputacao ---------------------------------------------------------------
log_subsection("Imputacao de valores ausentes")
n_na <- sum(is.na(df$AG_Ratio))
mediana_ag <- median(df$AG_Ratio, na.rm = TRUE)
df$AG_Ratio[is.na(df$AG_Ratio)] <- mediana_ag
log_info("AG_Ratio: %d valores imputados com a mediana (%.2f).", n_na, mediana_ag)
log_ok("Total de NAs restantes: %d", sum(is.na(df)))
...
salvar_rds(treino_proc, "data/df_proc.rds")   # treino pre-processado (entrada do SMOTE)
